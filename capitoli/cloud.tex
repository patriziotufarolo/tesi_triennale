...
Il termine \textit{cloud computing} indica un insieme di tecnologie informatiche che permettono l'accesso on-demand a un pool di risorse condivise (reti, server, storage, potenza computazionale, applicazioni, servizi) messe a disposizione su una rete eterogenea distribuita tramite tecniche di astrazione (es. API), virtualizzazione e contenimento, rilasciabili celermente con il minimo effort e la minima interazione da parte sia dell'utente, sia del fornitore di servizio. %ELIA il minimo effort da parte di chi? dell'utente? io scriverei il minimo impegno da parte dell'utente
...
\end{itemize}
\item \textbf{Terminazione del contratto}: occorre stabilire delle clausole di terminazione del contratto sia per garanzia del fornitore che dell'utilizzatore del servizio.
\end{itemize}
\end{document}